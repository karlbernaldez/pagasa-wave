...
export default function VerifyEmailPage() {
  ...
  useEffect(() => {
    ...
    verifyEmail(token, controller.signal)
      .then((data) => {
        ...
        setVerificationKind(data?.kind ?? null);
        setStatus('success');
      })
      .catch((err) => {
  ...
  const isEmailChange = status === 'success' && verificationKind === 'email_change';
  const cfg = isEmailChange
    ? {
        ...STATUS_CONFIG.success,
        title: 'Email Updated!',
        subtitle: 'Your new email is verified. Sign in again with the new address to continue.',
      }
        ...
        >
          <div
            style={{
              ...
              inset: 0,
              background:
                'linear-gradient(135deg, rgba(56,189,248,0.04) 0%, rgba(129,140,248,0.04) 100%)',
              borderRadius: 28,
              pointerEvents: 'none',
            }}
          />
          <div
            style={{
              height: 3,
              background: `linear-gradient(90deg, ${cfg.accent}, #818cf8)`,
          ...
          <div style={{ padding: '40px 40px 36px', position: 'relative' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {status === 'success' && (
                <button
                  onClick={() => navigate(isEmailChange ? '/login' : '/studio')}
                  style={{
                    width: '100%',
                    padding: '14px',
                    borderRadius: 12,
                ...
                >
                  {isEmailChange ? 'Sign in with new email →' : 'Continue to WaveLab →'}
                </button>
              )}
            ...
            {!isEmailChange && <Steps active={stepIndex} />}
          </div>
        </div>
