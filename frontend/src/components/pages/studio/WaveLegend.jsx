...
// Major numeric anchors only
const MAJOR_TICKS = [0, 1, 3, 5, 10, 14, 20];

// Qualitative ranges (grouped)
const QUAL_RANGES = [
  ...
  return (
    <div className="fixed bottom-6 right-6 z-40 select-none">
      ...
      >
        ...
        {/* QUALITATIVE SCALE (GROUPED) */}
        <div
          className={`mt-1 flex text-[9px] font-medium
          ${isDarkMode ? "text-slate-400" : "text-slate-600"}`}
        >
          {QUAL_RANGES.map((q) => {
            const span =
              HW_BINS.filter((b) => b >= q.from && b < q.to).length;
            return (
              <span
                key={q.label}
