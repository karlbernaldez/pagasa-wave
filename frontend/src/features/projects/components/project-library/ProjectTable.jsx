...
function ProjectMobileRow({ project, isDarkMode, isReviewMode, active, setActive, onOpen, onRename, onDelete }) {
    ...
    <article className={`rounded-2xl border p-4 shadow-sm ${isDarkMode ? "border-white/10 bg-slate-900/80" : "border-slate-200 bg-white"} ${status.needsRevision ? (isDarkMode ? "ring-1 ring-amber-400/20" : "ring-1 ring-amber-100") : ""}`}>
      ...
      <dl className="mt-4 grid grid-cols-2 gap-3 text-xs">
        ...
        <div className="min-w-0">
          ...
          <dd className={`mt-1 truncate font-semibold ${isDarkMode ? "text-slate-300" : "text-slate-700"}`}>{formatDate(project.updatedAt || project.submittedAt || project.createdAt, "MMM d, h:mm a")}</dd>
        </div>
      </dl>
      ...
      <div className={`relative mt-4 grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2 border-t pt-4 ${isDarkMode ? "border-white/10" : "border-slate-100"}`}>
        ...
        {hasMenuActions && (
          <Button className="shrink-0" variant="icon" size="sm" icon={MoreHorizontal} aria-label={`More actions for ${project.name}`} onClick={() => setActive(active === projectId ? null : projectId)} />
        )}
        ...
        {hasMenuActions && active === projectId && (
          <div className={`absolute right-0 bottom-full z-50 mb-2 w-36 rounded-lg border py-1 text-sm shadow-lg ${isDarkMode ? "border-white/10 bg-slate-950" : "border-slate-200 bg-white"}`}>
            {onRename && (
              <button type="button" onClick={() => { setActive(null); onRename(project); }} className={`flex w-full items-center gap-2 px-3 py-2 text-left font-semibold ${isDarkMode ? "text-slate-200 hover:bg-white/5" : "text-slate-700 hover:bg-slate-50"}`}>
                <Pencil size={14} aria-hidden="true" /> Rename
              </button>
            )}
            {onDelete && (
              <button type="button" onClick={() => { setActive(null); onDelete(project); }} className="flex w-full items-center gap-2 px-3 py-2 text-left font-semibold text-red-500 hover:bg-red-500/10">
...
export default function ProjectTable({ projects, loading, error, onRetry, onOpen, onRename, onDelete, mode = "library", isDarkMode = false }) {
  const [active, setActive] = useState(null);
  const isReviewMode = mode === "review";

  if (loading) {
    return <div className={`rounded-xl border p-6 text-sm ${isDarkMode ? "border-white/10 bg-slate-900/80 text-slate-400" : "border-slate-200 bg-white text-slate-500"}`}>Loading projects...</div>;
  }
  ...
  if (!projects || projects.length === 0) {
    return <div className={`rounded-xl border p-10 text-center text-sm ${isDarkMode ? "border-white/10 bg-slate-900/80 text-slate-400" : "border-slate-200 bg-white text-slate-500"}`}>No projects found</div>;
  }
    ...
    <>
      <div className="space-y-3 md:hidden">
        {projects.map((project) => (
          <ProjectMobileRow key={getProjectId(project)} project={project} isDarkMode={isDarkMode} isReviewMode={isReviewMode} active={active} setActive={setActive} onOpen={onOpen} onRename={onRename} onDelete={onDelete} />
        ))}
      </div>
              ...
              return (
                <tr key={projectId} className={`border-t transition ${isDarkMode ? "border-white/10 hover:bg-white/5" : "border-slate-100 hover:bg-slate-50"} ${status.needsRevision ? (isDarkMode ? "bg-amber-500/5" : "bg-amber-50/30") : ""}`}>
                  <td className="min-w-0 px-4 py-3">
                    ...
                    <p className={`mt-0.5 truncate text-xs font-semibold uppercase tracking-[0.12em] ${isDarkMode ? "text-slate-500" : "text-slate-400"}`}>{project.chartType || "Forecast"}</p>
                    {status.needsRevision && latestRemarks?.comment && (
                  ...
                  {isReviewMode && <td className={`min-w-0 px-4 py-3 ${isDarkMode ? "text-slate-300" : "text-slate-600"}`}><span className="block truncate">{project.ownerDisplay || "Project Owner"}</span></td>}
                  <td className={`px-4 py-3 ${isDarkMode ? "text-slate-300" : "text-slate-600"}`}><span className="block truncate">{formatDate(project.forecastDate)}</span></td>
                  <td className="px-4 py-3"><span className={`inline-flex max-w-full truncate rounded-full border px-2.5 py-1 text-xs font-bold ${status.className}`}>{status.label}</span></td>
                  <td className={`px-4 py-3 ${isDarkMode ? "text-slate-300" : "text-slate-600"}`}><span className="block truncate">{formatDate(project.updatedAt || project.submittedAt || project.createdAt, "MMM d, yyyy hh:mm a")}</span></td>
                  ...
                  <td className="relative px-4 py-3 text-right">
                    ...
                    {hasMenuActions && active === projectId && (
                      <div className={`absolute right-4 bottom-full z-50 mb-2 w-36 rounded-lg border py-1 text-sm shadow-lg ${isDarkMode ? "border-white/10 bg-slate-950" : "border-slate-200 bg-white"}`}>
                        {onRename && <button type="button" onClick={() => { setActive(null); onRename(project); }} className={`flex w-full items-center gap-2 px-3 py-2 text-left font-semibold ${isDarkMode ? "text-slate-200 hover:bg-white/5" : "text-slate-700 hover:bg-slate-50"}`}><Pencil size={14} aria-hidden="true" /> Rename</button>}
                        {onDelete && <button type="button" onClick={() => { setActive(null); onDelete(project); }} className="flex w-full items-center gap-2 px-3 py-2 text-left font-semibold text-red-500 hover:bg-red-500/10"><Trash2 size={14} aria-hidden="true" /> Delete</button>}
                      </div>
                    )}
                  </td>
                </tr>
