...
const DIFF_COLOR_EXPRESSION = ['match', ['get', 'diffStatus'], 'added', '#22c55e', 'changed', '#f97316', 'removed', '#ef4444', 'unchanged', '#64748b', '#0284c7'];
const styleGet = (key, fallback) => ['coalesce', ['get', key, ['get', 'style']], fallback];
const LINE_LABEL_TEXT = ['coalesce', ['to-string', ['get', 'labelValue']], ['to-string', ['get', 'waveHeight']], ['to-string', ['get', 'heightValue']], ['to-string', ['get', 'height']], ['to-string', ['get', 'value']], ['to-string', ['get', 'text']], ['to-string', ['get', 'label']], ['to-string', ['get', 'name']], ['to-string', ['get', 'title']], ''];
const LINE_LABEL_FILTER = ['all', ['match', ['geometry-type'], ['LineString', 'MultiLineString'], true, false], ['any', ['has', 'labelValue'], ['has', 'waveHeight'], ['has', 'heightValue'], ['has', 'height'], ['has', 'value'], ['has', 'text'], ['has', 'label'], ['has', 'name'], ['has', 'title']]];
...
const POINT_SYMBOL_FILTER = ['all', POINT_FILTER, ['!=', POINT_MARKER_TYPE, 'text_note'], ['!=', POINT_MARKER_TYPE, 'less_1']];
...
function getFeatureCacheKey(projectId, scope) { return `${scope || 'user'}:${projectId || 'none'}`; }
...
function hasDiffStyles(featureCollection) { return featureCollection.features.some((feature) => Boolean(feature?.properties?.diffStatus)); }
...
function getFeatureBounds(featureCollection) {
  ...
  featureCollection.features.forEach((feature) => extendBoundsFromCoordinates(bounds, feature?.geometry?.coordinates));
  return bounds.isEmpty() ? null : bounds;
}
function setLayerVisibility(map, layerId, isVisible) { if (map.getLayer(layerId)) map.setLayoutProperty(layerId, 'visibility', isVisible ? 'visible' : 'none'); }
function ensurePreviewImages(map) {
  const imageId = 'preview-less-1';
  if (!map || map.hasImage(imageId)) return;
  map.loadImage('/L1.png', (error, image) => { if (!error && image && !map.hasImage(imageId)) map.addImage(imageId, image); });
}
function getFeatureId(feature) {
  const props = feature?.properties || {};
  return feature?.sourceId || props.sourceId || props.stableId || props.annotationId || feature?.id || props.id || props.name || `preview-${Math.random().toString(36).slice(2)}`;
}
function normalizeFrontType(frontType) { return FRONT_TYPES[frontType] ? frontType : 'cold'; }
...
function triangleCoordinates(map, x, y, ux, uy, nx, ny, side) {
  ...
  return [[toLngLat(map, x - ux * size, y - uy * size), toLngLat(map, x + ux * size, y + uy * size), toLngLat(map, x + nx * side * size * 1.35, y + ny * side * size * 1.35), toLngLat(map, x - ux * size, y - uy * size)]];
}
function semicircleCoordinates(map, x, y, ux, uy, nx, ny, side) {
  const points = [];
  for (let i = 0; i <= 12; i += 1) {
    const theta = Math.PI - (Math.PI * i) / 12;
    points.push(toLngLat(map, x + ux * FRONT_SYMBOL_RADIUS * Math.cos(theta) + nx * side * FRONT_SYMBOL_RADIUS * Math.sin(theta), y + uy * FRONT_SYMBOL_RADIUS * Math.cos(theta) + ny * side * FRONT_SYMBOL_RADIUS * Math.sin(theta)));
  }
...
function buildShapeFeature(map, symbol, sideMultiplier, x, y, ux, uy, nx, ny) {
  ...
  const coordinates = symbol.kind === 'triangle' ? triangleCoordinates(map, x, y, ux, uy, nx, ny, side) : semicircleCoordinates(map, x, y, ux, uy, nx, ny, side);
...
function removePreviewFront(map, id) {
  [`${id}_dash`, `${id}_secondary`, `${id}_frontSymbols`].forEach((layerId) => { if (map.getLayer(layerId)) map.removeLayer(layerId); });
  [`${id}_frontSymbolSource`, `${id}_stationarySegmentSource`, id].forEach((sourceId) => { if (map.getSource(sourceId)) map.removeSource(sourceId); });
}
function renderFrontFeature(map, feature, showDiffStyles) {
  ...
  if (frontType === 'stationary') {
    setSourceData(map, `${id}_stationarySegmentSource`, { type: 'FeatureCollection', features: buildStationarySegmentFeatures(map, coordinates) });
    if (!map.getLayer(`${id}_dash`)) map.addLayer({ id: `${id}_dash`, type: 'line', source: `${id}_stationarySegmentSource`, layout: { 'line-join': 'round', 'line-cap': 'butt' }, paint: { 'line-color': ['get', 'color'], 'line-width': lineWidth, 'line-opacity': style.lineOpacity ?? 1 } });
  } else if (!map.getLayer(`${id}_dash`)) {
    map.addLayer({ id: `${id}_dash`, type: 'line', source: id, layout: { 'line-join': 'round', 'line-cap': 'round' }, paint: { 'line-color': lineColor, 'line-width': lineWidth, 'line-opacity': style.lineOpacity ?? 1 } });
  } else {
    map.setPaintProperty(`${id}_dash`, 'line-color', lineColor);
    map.setPaintProperty(`${id}_dash`, 'line-width', lineWidth);
  }

  setSourceData(map, `${id}_frontSymbolSource`, { type: 'FeatureCollection', features: frontType === 'stationary' ? buildStationarySymbolFeatures(map, coordinates, sideMultiplier) : buildFrontSymbolFeatures(map, coordinates, frontType, sideMultiplier) });
  if (!map.getLayer(`${id}_frontSymbols`)) map.addLayer({ id: `${id}_frontSymbols`, type: 'fill', source: `${id}_frontSymbolSource`, paint: { 'fill-color': ['get', 'color'], 'fill-opacity': style.symbolOpacity ?? style.fillOpacity ?? 1 } });
}
function renderFrontFeatures(map, features, showDiffStyles) {
  ...
  previousIds.forEach((id) => { if (!ids.has(id)) removePreviewFront(map, id); });
...
function setPreviewLayerPaint(map, { showDiffStyles, styleMode }) {
  ...
  if (map.getLayer('project-preview-polygons')) {
    ...
    map.setPaintProperty('project-preview-polygons', 'fill-opacity', showDiffStyles ? ['match', ['get', 'diffStatus'], 'unchanged', 0.2, 0.42] : styleGet('fillOpacity', paint.polygonOpacity));
  }
  if (map.getLayer('project-preview-polygons-outline')) {
    map.setPaintProperty('project-preview-polygons-outline', 'line-color', showDiffStyles ? lineColor : styleGet('lineColor', paint.polygonOutline));
    map.setPaintProperty('project-preview-polygons-outline', 'line-width', styleGet('lineWidth', paint.polygonOutlineWidth));
  ...
  if (map.getLayer('project-preview-lines-casing')) {
    map.setPaintProperty('project-preview-lines-casing', 'line-color', styleGet('lineCasing', paint.lineCasing));
    map.setPaintProperty('project-preview-lines-casing', 'line-width', styleGet('lineCasingWidth', paint.lineCasingWidth));
  }
  ...
  if (map.getLayer('project-preview-points-halo')) {
    ...
    map.setPaintProperty('project-preview-points-halo', 'circle-opacity', paint.showPoints ? styleGet('iconOpacity', 0.28) : 0);
  }
  if (map.getLayer('project-preview-points-symbol')) {
    map.setPaintProperty('project-preview-points-symbol', 'circle-color', pointColor);
    map.setPaintProperty('project-preview-points-symbol', 'circle-radius', styleGet('iconSize', paint.pointRadius));
    map.setPaintProperty('project-preview-points-symbol', 'circle-stroke-color', styleGet('pointStroke', paint.pointStroke));
    map.setPaintProperty('project-preview-points-symbol', 'circle-stroke-width', styleGet('pointStrokeWidth', paint.pointStrokeWidth));
    map.setPaintProperty('project-preview-points-symbol', 'circle-opacity', paint.showPoints ? styleGet('iconOpacity', 1) : 0);
  }
  if (map.getLayer('project-preview-points-label')) {
    ...
    map.setPaintProperty('project-preview-points-label', 'text-color', styleGet('textColor', paint.labelColor));
    map.setPaintProperty('project-preview-points-label', 'text-halo-color', styleGet('textHaloColor', paint.labelHaloColor));
    map.setPaintProperty('project-preview-points-label', 'text-halo-width', styleGet('textHaloWidth', paint.labelHaloWidth));
  }
}
function addPreviewLayers(map, featureCollection, { showLabels = true, showDiffStyles = false, styleMode } = {}) {
  ...
  map.addLayer({ id: 'project-preview-lines-casing', type: 'line', source: sourceId, filter: ['match', ['geometry-type'], ['LineString', 'MultiLineString'], true, false], paint: { 'line-color': styleGet('lineCasing', paint.lineCasing), 'line-width': styleGet('lineCasingWidth', paint.lineCasingWidth), 'line-opacity': 0.95 } });
...
async function loadProjectFeaturesCached(projectId, scope) {
  ...
  const request = loadProjectFeatures(projectId, scope).then((data) => { setCachedFeatures(key, data); return data; }).finally(() => featureRequestCache.delete(key));
...
function ProjectPreviewMap({ projectId, features, featureScope = 'user', className = '', height = 180, isDarkMode = false, emptyLabel = 'No annotations yet', lazy = true, showLabels = true, showDiffStyles = false, chartStyleMode, fixedBounds = false }) {
  ...
  const featureCollection = useMemo(() => (hasProvidedFeatures ? providedFeatureCollection : normalizeFeatureCollection(remoteFeatures)), [hasProvidedFeatures, providedFeatureCollection, remoteFeatures]);
  ...
  useEffect(() => {
    ...
    const map = new mapboxgl.Map({ container: containerRef.current, style: STYLE_URL, projection: 'mercator', center: DEFAULT_CENTER, zoom: 4.8, interactive: false, attributionControl: false, preserveDrawingBuffer: false, fadeDuration: 0 });
    mapRef.current = map;
    map.on('load', () => { setIsReady(true); map.resize(); map.fitBounds(DEFAULT_BOUNDS, { padding: 18, maxZoom: 6, duration: 0 }); });
    return () => { map.remove(); mapRef.current = null; fittedFeaturesKeyRef.current = ''; setIsReady(false); };
  }, [shouldRenderMap]);
  useEffect(() => { if (shouldRenderMap || !mapRef.current) return; mapRef.current.remove(); mapRef.current = null; fittedFeaturesKeyRef.current = ''; setIsReady(false); }, [shouldRenderMap]);
  useEffect(() => {
    ...
    if (fixedBounds) { map.fitBounds(DEFAULT_BOUNDS, { padding: 18, maxZoom: 6, duration: 0 }); fittedFeaturesKeyRef.current = featureKey; return; }
    const bounds = getFeatureBounds(featureCollection);
    if (bounds) { map.fitBounds(bounds, { padding: 72, maxZoom: featureCollection.features.length === 1 ? 8 : 10, duration: 0 }); fittedFeaturesKeyRef.current = featureKey; }
  }, [featureCollection, featureKey, fixedBounds, hasFeatures, isReady, normalizedStyleMode, shouldUseDiffStyles, showLabels]);

  return (
    <div ref={viewportRef} className={`relative overflow-hidden rounded-xl border transition-colors ${isDarkMode ? 'border-white/10 bg-slate-900' : 'border-slate-200 bg-slate-100'} ${className}`} style={containerStyle}>
      {shouldRenderMap ? <div ref={containerRef} className="h-full w-full" aria-hidden="true" /> : <div className={`h-full w-full ${isDarkMode ? 'bg-slate-900' : 'bg-slate-100'}`} aria-hidden="true" />}
      {!isNearViewport && <div className={`h-full w-full animate-pulse ${isDarkMode ? 'bg-slate-800' : 'bg-slate-200'}`} aria-hidden="true" />}
