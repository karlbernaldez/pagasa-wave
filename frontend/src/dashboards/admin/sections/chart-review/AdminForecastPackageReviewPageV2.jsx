...
function getEmptyStateCopy({ hasFilters }) {
  if (hasFilters) {
    return {
      ...
      body: 'Clear a filter or search another forecast date, forecaster, or chart type.',
    };
  }

  return {
    title: 'The review queue is clear.',
    body: 'Submitted and under-review forecast packages will appear here when administrator attention is required.',
  };
}

function StateCard({ children, isDarkMode }) {
  return (
    <div className={`rounded-2xl border p-10 text-center shadow-xl backdrop-blur-2xl ${isDarkMode ? 'border-white/10 bg-slate-950/48 text-slate-200 shadow-black/20' : 'border-white/75 bg-white/68 text-slate-800 shadow-slate-300/35'}`}>
      {children}
    </div>
  );
}
...
export default function AdminForecastPackageReviewPageV2() {
  ...
  return (
    <div className="min-h-full bg-transparent">
      <div className="mx-auto max-w-[1500px] space-y-4 p-4 sm:p-6">
        ...
        {!query.isLoading && query.error && (
          <StateCard isDarkMode={isDarkMode}>
            <p className="mb-4 text-sm font-semibold">Forecast packages could not be loaded.</p>
            <Button onClick={() => query.refetch()}>Retry</Button>
          </StateCard>
        )}

        {!query.isLoading && !query.error && visiblePackages.length === 0 && (
          <StateCard isDarkMode={isDarkMode}>
            <FolderKanban className="mx-auto mb-3" size={26} />
            <h2 className="text-lg font-black">{emptyStateCopy.title}</h2>
            <p className={`mx-auto mt-2 max-w-md text-sm font-semibold ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>{emptyStateCopy.body}</p>
            {hasActiveFilters && <div className="mt-5"><Button variant="secondary" onClick={resetFilters}>Clear filters</Button></div>}
          </StateCard>
        )}

        {!query.isLoading && !query.error && visiblePackages.length > 0 && (
          <div className="grid gap-4 xl:grid-cols-2">
            {visiblePackages.map((forecastPackage) => (
              <ForecastPackageCard
      ...
      <ProjectReviewModal
        project={reviewProject ? { ...reviewProject, reviewComment: undefined } : null}
        isDarkMode={isDarkMode}
        onClose={() => setReviewProject(null)}
        onApprove={(project) => approveProject(getId(project))}
        onReject={(project, remarks) => rejectProject(getId(project), remarks)}
