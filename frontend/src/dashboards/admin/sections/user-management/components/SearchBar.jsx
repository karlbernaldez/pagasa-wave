 ...
 */
export function SearchBar({ query, onQueryChange, statusFilter, onStatusChange, isDarkMode }) {
  ...
  return (
    <div className="flex flex-col sm:flex-row gap-3 mb-6">
      ...
      >
        ...
        {query && (
          <button
            ...
            className={`text-xs px-1.5 py-0.5 rounded transition-colors ${isDarkMode ? 'text-slate-400 hover:text-slate-200' : 'text-slate-400 hover:text-slate-600'}`}
          >
            ✕
          </button>
        )}
      ...
      {/* Status filter pills */}
      <div className={`flex items-center gap-1 px-1.5 py-1.5 rounded-xl border ${isDarkMode ? 'bg-slate-900/70 border-slate-700/60' : 'bg-slate-50 border-slate-200'}`}>
        <SlidersHorizontal size={14} className={`mx-1.5 shrink-0 ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`} />
