      ...
      <Accordion icon={Globe} title="Public Content" dark={dark} defaultOpen>
        <div className="grid gap-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Field label="Alert Email" type="email" value={settings.contactEmail}  onChange={set('contactEmail')}  dark={dark} />
            <Field label="Default Region"             value={settings.defaultRegion} onChange={set('defaultRegion')} dark={dark} />
          </div>
        </div>
      </Accordion>
        ...
        <div className="flex flex-col gap-4">
          ...
          {settings.logoPreview && (
            <div className={`p-4 rounded-xl border ${
              ...
              <p className={`text-xs mb-3 ${dark ? 'text-slate-500' : 'text-slate-400'}`}>Preview</p>
              <img src={settings.logoPreview} alt="Logo" className="h-12 w-auto object-contain" />
            </div>
          )}
        </div>
      </Accordion>
        ...
        <div className="flex flex-col gap-4">
          ...
          <label className={`flex items-center gap-3 p-4 rounded-xl border cursor-pointer transition-all duration-200 ${
            settings.maintenanceMode
              ? dark ? 'border-amber-500/40 bg-amber-500/5' : 'border-amber-300 bg-amber-50'
            ...
            <div className={`relative w-11 h-6 rounded-full transition-colors duration-300 ${
              settings.maintenanceMode ? 'bg-amber-500' : dark ? 'bg-slate-700' : 'bg-slate-300'
            }`}>
