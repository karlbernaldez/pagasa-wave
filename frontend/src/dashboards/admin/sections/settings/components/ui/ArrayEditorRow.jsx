...
// ── Row wrapper ────────────────────────────────────────
export const ArrayRow = ({ index, onRemove, dark, children }) => (
  <div className={`group relative flex gap-3 items-start p-4 rounded-xl border transition-all duration-200 ${
    dark
      ? 'bg-slate-800/50 border-slate-700/60 hover:border-slate-600'
      : 'bg-slate-50 border-slate-200 hover:border-slate-300'
  }`}>
    {/* Drag handle hint */}
    <div className={`flex-shrink-0 mt-1 cursor-grab opacity-30 group-hover:opacity-60 ${
    ...
    <button
      ...
      className={`flex-shrink-0 mt-0.5 p-1.5 rounded-lg opacity-0 group-hover:opacity-100 transition-all duration-200 hover:scale-110 ${
...
export const AddButton = ({ onClick, label, dark }) => (
  <button
    onClick={onClick}
    className={`w-full flex items-center justify-center gap-2 py-3 rounded-xl border-2 border-dashed text-sm font-semibold transition-all duration-200 hover:scale-[1.01] ${
      dark
