    ...
    <Card isDarkMode={isDarkMode} className="p-4">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          ...
          <p
            className={cn('mt-1 text-lg font-black', isDarkMode ? 'text-white' : 'text-slate-950')}
          >
            {value}
          </p>
          {helper && (
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-3">
              {[
                ...
                [
                  'Builder / runtime',
                  model.builderConfigured || model.runtimeConfigured
                    ? 'Configured'
                    : 'Not configured',
                ],
              ].map(([label, value]) => (
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className={cn('mt-1 text-xs', isDarkMode ? 'text-slate-400' : 'text-slate-500')}>
              Configuration is model-specific. Dedicated WW3 and ECWAM operational controls remain
              separate from managed custom runtime profiles.
            </p>
...
function PackagesTab({ model, busyKey, isDarkMode, onDeletePackages }) {
  ...
  return (
    <Card isDarkMode={isDarkMode} className="overflow-hidden">
      <div
        className={cn('border-b p-4 sm:p-5', isDarkMode ? 'border-white/10' : 'border-slate-200')}
      >
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className={cn('mt-1 text-xs', isDarkMode ? 'text-slate-400' : 'text-slate-500')}>
              Packages are protected until they are older than {model.packageRetentionDays || 10}{' '}
              days. Retention is enforced by the backend.
            </p>
          </div>
          <button
            type="button"
      ...
      {filtered.length === 0 ? (
        ...
        <>
          <div className="overflow-x-auto">
            <table className="w-full min-w-[850px] text-left text-xs">
              <thead>
                <tr
                  className={
                    isDarkMode ? 'bg-black/10 text-slate-500' : 'bg-slate-50 text-slate-500'
                  }
                >
          ...
          <div
            className={cn(
              'flex flex-col gap-3 border-t px-4 py-3 text-xs sm:flex-row sm:items-center sm:justify-between',
          ...
          >
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className={cn(
                  'rounded-lg border px-3 py-1.5 font-black disabled:opacity-40',
                  isDarkMode ? 'border-white/10 text-slate-300' : 'border-slate-200 text-slate-700'
                )}
              >
                Previous
              </button>
...
function ActivityTab({ model, isDarkMode }) {
  ...
  const events = [
    {
      ...
      value: operations?.service?.running
        ? 'Running now'
        : operations?.service?.result || 'No completed result',
      helper:
  ...
  return (
    <Card isDarkMode={isDarkMode} className="p-5">
      ...
      <p className={cn('mt-1 text-xs', isDarkMode ? 'text-slate-400' : 'text-slate-500')}>
        Current operational signals reported by the model service. This is not a replacement for the
        audit log.
      </p>
...
export default function WaveModelDetail({
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <h2
                className={cn('text-xl font-black', isDarkMode ? 'text-white' : 'text-slate-950')}
              >
                {model.label}
              </h2>
              <span
      ...
      <div
        className={cn(
          'flex overflow-x-auto rounded-xl border p-1',
      ...
      {tab === 'configuration' && (
        <ConfigurationTab key={model.code} model={model} isDarkMode={isDarkMode} />
      )}
      {tab === 'packages' && (
        <PackagesTab
          model={model}
