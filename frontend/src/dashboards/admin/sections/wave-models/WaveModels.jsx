...
function StatCard({ icon: Icon, label, value, helper, isDarkMode }) {
  return (
    ...
    >
      ...
      <p className={cn('mt-2 text-xs font-semibold', isDarkMode ? 'text-slate-500' : 'text-slate-500')}>
        {helper}
      </p>
    </div>
  );
...
export default function WaveModelsSection({ isDarkMode = true }) {
  ...
  useEffect(() => {
    if (selectedCode && !loading && !selectedModel) setSelectedCode(null);
  }, [loading, selectedCode, selectedModel]);
        ...
        <>
          ...
          <section className={cn('overflow-hidden rounded-2xl border shadow-xl backdrop-blur-xl', isDarkMode ? 'border-white/10 bg-slate-950/50 shadow-black/20' : 'border-white/70 bg-white/70 shadow-slate-300/40')}>
            ...
            <div className="px-4 py-4 sm:px-5">
              ...
              <div className={cn('mt-4 flex items-start gap-2 border-t pt-3 text-[10px] leading-relaxed', isDarkMode ? 'border-white/[0.08] text-slate-600' : 'border-slate-100 text-slate-400')}>
                <Database size={12} className="mt-0.5 shrink-0" />
                Package deletion is only available from a model's Packages tab. The backend independently enforces the minimum package age before filesystem removal.
              </div>
            </div>
          </section>
        </>
