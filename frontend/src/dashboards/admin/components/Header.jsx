  ...
  return (
    <header
      className={`border-b sticky top-0 z-20 backdrop-blur-xl ${
        isDarkMode ? 'bg-gray-800/60 border-gray-700/50' : 'bg-white/70 border-gray-200/60'
      }`}
    >
      <div className="px-4 md:px-6 py-4 flex flex-col gap-4">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <button
              ...
              className={`lg:hidden p-2 rounded-lg transition-all ${isDarkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-100'}`}
              aria-label="Open menu"
            >
              <Menu size={22} className={isDarkMode ? 'text-gray-300' : 'text-gray-600'} />
            </button>
            <div>
              <h2 className={`text-2xl md:text-3xl font-black ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{activeMeta.title}</h2>
              <p className={`text-sm mt-1 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>{activeMeta.description}</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 md:gap-3">
            ...
            <div className="relative">
              <button
                ...
                className={`relative p-3 rounded-xl transition-colors ${
                  isDarkMode ? 'bg-gray-700/60 text-gray-200 hover:bg-gray-700' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                }`}
              ...
              {showNotificationDropdown && (
                <>
                  ...
                  <div
                    className={`absolute right-0 mt-3 w-80 rounded-2xl shadow-2xl border backdrop-blur-xl z-20 overflow-hidden ${
                      isDarkMode ? 'bg-gray-800/90 border-gray-700/50' : 'bg-white/95 border-gray-200/70'
                    }`}
                  >
                    <div className={`px-4 py-3 border-b ${isDarkMode ? 'border-gray-700/60' : 'border-gray-200/70'}`}>
                      <p className={`font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Notifications</p>
                    </div>
                    <div className="max-h-72 overflow-y-auto">
                      {NOTIFICATIONS.map((item) => (
                        <div key={item.id} className={`px-4 py-3 border-b last:border-b-0 ${isDarkMode ? 'border-gray-700/50' : 'border-gray-100'}`}>
                          <p className={`text-sm font-semibold ${isDarkMode ? 'text-gray-100' : 'text-gray-800'}`}>{item.title}</p>
                          <p className={`text-xs mt-1 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>{item.time}</p>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            <div className="relative">
              <button
                ...
                className={`flex items-center gap-3 px-2 md:px-3 py-2 rounded-xl transition-all duration-300 hover:scale-105 ${
              ...
              {showUserDropdown && (
                <>
                  ...
                  <div
                    className={`absolute right-0 mt-3 w-64 rounded-2xl shadow-2xl border backdrop-blur-xl z-20 overflow-hidden ${
                      isDarkMode ? 'bg-gray-800/80 border-gray-700/50' : 'bg-white/90 border-gray-200/70'
                    }`}
                  >
        ...
        <div
          className={`flex items-center gap-3 px-4 py-3 rounded-xl border ${
            isDarkMode ? 'bg-gray-900/70 border-gray-700' : 'bg-white border-gray-200'
          }`}
        >
          ...
          <input
            ...
            placeholder="Search users, charts, schedules..."
            className={`w-full bg-transparent outline-none text-sm ${
              isDarkMode ? 'text-gray-100 placeholder:text-gray-500' : 'text-gray-800 placeholder:text-gray-400'
            }`}
          />
        </div>
      </div>
