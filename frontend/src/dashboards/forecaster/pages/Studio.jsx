...
const Studio = ({ logger }) => {
  ...
  const loadChartContext = useCallback(async ({ signal, silent = false, autoJoin } = {}) => {
    ...
    try {
      setChartContext(await fetchForecastPackageChartContextByProject(projectId, { signal, autoJoin: shouldAutoJoin }));
    } catch (error) {
  ...
  useEffect(() => {
    const handler = (event) => {
      if (String(event.detail?.projectId || "") === String(projectId)) {
        loadChartContext({ silent: true, autoJoin: false });
      }
    };
    window.addEventListener(FORECAST_CHART_BROWSER_EVENT, handler);
    return () => window.removeEventListener(FORECAST_CHART_BROWSER_EVENT, handler);
  }, [loadChartContext, projectId]);
  ...
  useEffect(() => {
    ...
    setShowTitleModal(false);
    selectedToolRef.current = null;
  }, [canUseEditingTools, isCanvasActive, isFlagCanvasActive, toggleCanvas, toggleFlagCanvas, setShowTitleModal]);
  ...
  useEffect(() => {
    ...
    ["wave-raster", "wave-glass-fill", "wave-glass-depth", "wave-arrows", "wind-raster", "wind-particles", "wind-arrows", "wind-glass-fill", "wind-glass-depth"].forEach((id) => {
      if (map.getLayer(id)) map.removeLayer(id);
    });
    ["wave-dark", "wave-light", "wind-darkstorm", "wind-solarstorm"].forEach((id) => {
  ...
  const handleClaimChart = async () => {
    ...
    setStudioError("");
    try {
      setChartContext(await claimForecastPackageChartByProject(projectId));
    } catch (error) {
      setStudioError(error?.message || "Failed to join chart editing.");
    } finally {
  ...
  const handleReleaseChart = async () => {
    ...
    setStudioError("");
    try {
      setChartContext(await releaseForecastPackageChartByProject(projectId));
    } catch (error) {
      suppressAutoJoinRef.current = false;
      setStudioError(error?.message || "Failed to release chart editing session.");
    } finally {
  ...
  const handleCertifyChartReady = async () => {
    ...
    setStudioError("");
    try {
      setChartContext(await updateForecastChartCompletionByProject(projectId, true));
    } catch (error) {
  ...
  const handleReopenChartEdits = async () => {
    ...
    setStudioError("");
    try {
      setChartContext(await updateForecastChartCompletionByProject(projectId, false));
    } catch (error) {
  ...
  return (
    <div className={`relative h-screen w-full overflow-hidden ${isDarkMode ? "bg-slate-950" : "bg-slate-100"}`}>
        ...
        <div className="flex shrink-0 items-center gap-1 sm:gap-2">
          {isReadOnlyProject && (
            <span className={`hidden items-center gap-1.5 rounded-full border px-3 py-1 text-xs font-black xl:inline-flex ${isDarkMode ? "border-white/10 bg-slate-900 text-slate-300" : "border-slate-200 bg-slate-100 text-slate-600"}`}>
              <Lock size={13} />{isPackageReadyReadOnly ? "Chart ready" : canEditProject ? "Join editing to edit" : "Read-only"}
            </span>
          )}
          ...
          {chartContext && !isLoadingChartContext && !chartContext.blockingChartType && isPackageChartReady && (
            <span className={`studio-liquid-control hidden items-center gap-1.5 rounded-full border px-3 py-1 text-xs font-black xl:inline-flex ${isDarkMode ? "border-emerald-300/20 bg-emerald-400/10 text-emerald-200" : "border-emerald-200/80 bg-emerald-50/80 text-emerald-700"}`}>
              <CheckCircle2 size={13} />Certified ready
            </span>
          )}
          ...
          {chartContext && !isPackageChartReady && chartContext.blockingChartType && (
            <span className={`studio-liquid-control hidden rounded-full border px-3 py-1 text-xs font-black xl:inline-flex ${isDarkMode ? "border-slate-700 bg-slate-900 text-slate-400" : "border-slate-200 bg-slate-100 text-slate-600"}`}>
              {blockingChartLabel} must be ready first
            </span>
          )}

          {chartContext && !isPackageChartReady && !chartContext.blockingChartType && (
            <div className={`studio-liquid-control hidden items-center gap-1 rounded-2xl border p-1 md:flex ${headerControlGroup}`}>
              <span className={`px-2 text-[11px] font-black uppercase tracking-wide ${headerMutedText}`}>{chartLabel}</span>
              {chartClaim?.claimedByCurrentUser ? (
                <>
                  <span className={`rounded-xl px-2 py-1 text-xs font-black ${isDarkMode ? "bg-emerald-400/12 text-emerald-200" : "bg-emerald-50 text-emerald-700"}`}>You are editing</span>
                  <Button variant="secondary" size="sm" onClick={handleReleaseChart} disabled={isChartActionBusy} className={`!rounded-xl ${headerGhostButton}`}>{isReleasingChart ? "Releasing…" : "Release"}</Button>
                </>
              ) : chartClaim?.claimedByOtherUser ? (
                <>
                  <span className={`rounded-xl px-2 py-1 text-xs font-black ${isDarkMode ? "bg-cyan-400/10 text-cyan-200" : "bg-blue-50 text-blue-700"}`}>{activeEditorText || "Others editing"}</span>
                  <Button variant="secondary" size="sm" onClick={handleClaimChart} disabled={isChartActionBusy || !chartClaim?.canClaim} className={`!rounded-xl ${headerGhostButton}`}>{isClaimingChart ? "Joining…" : "Join editing"}</Button>
                </>
              ) : (
                <Button variant="secondary" size="sm" onClick={handleClaimChart} disabled={isChartActionBusy || !chartClaim?.canClaim} className={`!rounded-xl ${headerPrimaryButton}`}>{isClaimingChart ? "Joining…" : "Join editing"}</Button>
              )}
              <Button variant="secondary" size="sm" icon={CheckCircle2} onClick={handleCertifyChartReady} disabled={isChartActionBusy || !chartClaim?.canCertify} className={`!rounded-xl ${headerGhostButton}`}>Ready</Button>
            </div>
          )}

          {chartContext && isPackageChartReady && (
            <Button variant="secondary" size="sm" onClick={handleReopenChartEdits} disabled={isChartActionBusy} className={`hidden !rounded-xl md:inline-flex ${headerGhostButton}`}>Reopen</Button>
          )}
          <NotificationBell isDarkMode={isDarkMode} />
          <Button variant="secondary" size="sm" icon={isDarkMode ? Sun : Moon} onClick={handleToggleTheme} className={`!rounded-xl !px-2 ${headerGhostButton}`} aria-label="Toggle theme" />
        </div>
      </header>

      {studioError && (
        <div className="absolute left-1/2 top-[72px] z-[130] flex -translate-x-1/2 items-center gap-2 rounded-2xl border border-red-200 bg-red-50 px-4 py-2 text-sm font-bold text-red-700 shadow-xl">
          <AlertTriangle size={16} />{studioError}
        </div>
      )}

      <div className="absolute inset-x-0 bottom-0 z-[2]" style={{ top: STUDIO_HEADER_HEIGHT }}>
        <MapComponent onMapLoad={handleMapLoad} isDarkMode={isDarkMode} />
      </div>

      {showMainUI && canUseEditingTools && (
        <>
          {showToolbar && (
            <DrawToolBar
              ...
              toggleFlagCanvas={toggleFlagCanvas}
              lineCount={lineCount}
          ...
          <LayerPanel layers={layers} setLayers={setLayers} mapRef={mapRef} isDarkMode={isDarkMode} disabled={!canUseEditingTools} />
          <MapStatusBar mapRef={mapRef} isDarkMode={isDarkMode} />
        </>
      )}

      {isLoading && <MapLoading isLoading message="Finalizing setup" />}
      {showTitleModal && canUseEditingTools && <MarkerTitleModal markerTitle={markerTitle} handleTitleChange={handleTitleChange} handleSaveTitle={handleSaveTitle} closeModal={closeModal} markerType={type} />}
      {isCanvasActive && canUseEditingTools && (
        <Canvas
          mapRef={mapRef}
          drawCounter={drawCounter}
          setDrawCounter={setDrawCounter}
      ...
      {isFlagCanvasActive && canUseEditingTools && <FlagCanvas mapRef={mapRef} isDarkMode={isDarkMode} />}
      {showNoProjectsModal && <NoProjectAlert message={message} onCreate={handleOpenCreateProject} onMaybeLater={handleMaybeLater} />}
      {showCreateProjectModal && <CreateProjectModal closeModal={() => setShowCreateProjectModal(false)} onCreate={(projectName) => handleCreateProject(projectName, (project) => { setCurrentProject(project); setShowCreateProjectModal(false); navigate(`/studio/${project._id || project.id}`); })} />}

      {isInactivityPromptVisible && (
        <div className="absolute inset-0 z-[240] flex items-center justify-center bg-slate-950/45 p-4 backdrop-blur-sm">
          <div className={`w-full max-w-md rounded-3xl border p-6 shadow-2xl ${isDarkMode ? "border-white/10 bg-slate-950 text-slate-100" : "border-white bg-white text-slate-900"}`}>
            <p className="text-lg font-black">Workspace refresh needed</p>
            <p className={`mt-2 text-sm font-semibold ${isDarkMode ? "text-slate-400" : "text-slate-600"}`}>We paused the automatic reload because a drawing panel or modal is open. Continue your work or refresh now to sync the latest project data.</p>
            <div className="mt-5 flex justify-end gap-2"><Button variant="secondary" onClick={stayActive}>Keep working</Button><Button variant="primary" onClick={refreshWorkspace}>Refresh now</Button></div>
          </div>
        </div>
      )}
    </div>
