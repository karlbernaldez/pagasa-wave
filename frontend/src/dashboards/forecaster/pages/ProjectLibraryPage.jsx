...
const WARNING_CHART_STATUSES = new Set([
  "Revision Requested",
  "Under Review",
  "Submitted",
]);
...
const LOCKED_PACKAGE_COPY = {
  Submitted: {
    nextAction:
      "No action is needed right now. You will be notified if Admin requests revisions.",
  ...
  "Under Review": {
    nextAction:
      "No action is needed while Admin reviews the package. You will be notified of the decision.",
  ...
  Approved: {
    ...
    lockedNotice: "Approved and ready for publication.",
    currentStep: "Approved",
    reviewGateDetail: "Ready for publication",
  },
  Published: {
    nextAction:
      "Use the published chart output for sharing and operational decisions.",
    lockedNotice: "Published as a final operational output.",
    currentStep: "Published",
    reviewGateDetail: "Published output",
  },
  Rejected: {
    nextAction:
      "This review cycle is closed. Create or wait for the next forecast package.",
    lockedNotice: "Rejected by Admin; review cycle closed.",
    currentStep: "Rejected",
    reviewGateDetail: "Review closed",
  },
  "No Publication": {
    nextAction: "No further action is needed for this package.",
    lockedNotice: "Closed with no publication decision.",
    currentStep: "No Publication",
    reviewGateDetail: "No publication decision",
  },
  Archived: {
    nextAction: "Open the current active package to continue forecasting.",
    lockedNotice: "Archived as a read-only record.",
...
const CHART_METADATA = {
  ...
  forecast_48h: {
    ...
    mandate:
      "48-hour wave height and direction forecast for extended marine operations planning.",
...
function getReadyEditorNames(chart, completion) {
  const names = (Array.isArray(chart?.readyEditors) ? chart.readyEditors : [])
    ...
    .filter(Boolean);
  const readyByName = getUserDisplayName(
    chart?.readyBy || completion?.completedBy,
  );
...
function getChartProjectStatus(chart) {
  if (chart?.project && typeof chart.project === "object")
    return chart.project.status || "";
  return chart?.status || "";
}

function getChartCompletion(packageData, chartType) {
  return packageData?.chartCompletion?.find(
    (row) => row.chartType === chartType,
  );
}
...
function isRevisionActionPending(packageData, chart) {
  ...
  const revisionRequestedAt = getTimeValue(
    packageData.reviewedAt || packageData.updatedAt,
  );
  const completedAt = getTimeValue(completion.completedAt || chart?.readyAt);
  return (
    !completedAt || !revisionRequestedAt || completedAt <= revisionRequestedAt
  );
}
...
function getNextIncompleteChartType(packageData) {
  return (
    REQUIRED_CHART_SEQUENCE.find(
      (chartType) => !isChartComplete(packageData, chartType),
    ) || null
  );
}
...
function getSubmitReadiness(
  ...
  if (!packageData) {
    return {
      ...
      detail:
        "Create today's package to begin the four-chart forecast workflow.",
      tone: "neutral",
    };
  }
  if (!isEditable) {
    ...
    return {
      title: "Locked for forecasters",
      detail: copy.lockedNotice,
      tone: "locked",
    };
  }
  ...
  return {
    ...
    detail:
      "All required charts are certified. Submit this package for Admin review.",
    tone: "ready",
  };
}
    ...
    >
      ...
      <div className="flex min-w-0 items-start gap-3">
        ...
        <div className="relative min-w-0">
          <p
            className={`text-base font-black ${isDarkMode ? "text-cyan-300" : "text-blue-700"}`}
          >
            Next action
          </p>
          <p
...
function ChartCard({
  ...
  const chartReviewStatus =
    chartProjectStatus ||
    (APPROVED_PACKAGE_STATUSES.has(packageData?.status)
      ? "Approved"
      : packageData?.status);
  ...
  const isReCertifiedRevision =
    chartReviewStatus === REVISION_REQUESTED_STATUS &&
    isComplete &&
    !revisionActionPending;
  const displayChartStatus = isReCertifiedRevision
    ? "Re-certified"
    : chartReviewStatus;
  const blockingChartType = getFirstIncompletePrerequisite(
    packageData,
    chartType,
  );
  ...
  const readyEditorText = formatNameList(
    getReadyEditorNames(chart, completion),
  );
  ...
  const isWarningChart =
    ["Revision Requested", "Under Review"].includes(displayChartStatus) &&
    !isReCertifiedRevision;
  ...
  const statusLabel =
    isQueued && !displayChartStatus
      ? "Queued"
      : displayChartStatus ||
        (isComplete ? "Ready for review" : "In production");
      ...
      <div className="grid min-h-[156px] gap-3 p-4 sm:grid-cols-[28px_1fr] xl:grid-cols-[28px_68px_minmax(0,1fr)_172px] xl:items-center">
        ...
        >
          <div className="text-center">
            ...
            <span className="block text-[9px] font-black leading-none">
              {metadata.code}
            </span>
          </div>
        </div>
...
function PackageSummary({
  ...
  const ReadinessIcon = !isEditable
    ? LockKeyhole
    : completion.isComplete
      ? ShieldCheck
      : Waves;
  const statusIsComplete = ["Submitted", "Approved", "Published"].includes(
    packageData.status,
  );
  const lockedDescription =
    "The package is locked and cannot be edited until the Admin completes the review.";
...
function EmptyPackageState({ isCreating, isDarkMode, onCreate, message }) {
  return (
    ...
    >
      ...
      <h2
        className={`mt-5 text-xl font-black ${isDarkMode ? "text-slate-100" : "text-slate-900"}`}
      >
...
export default function ForecasterProjectLibraryPage() {
  ...
  const isEditable = EDITABLE_PACKAGE_STATUSES.has(
      ...
      <div className="relative mx-auto max-w-[1540px] space-y-5 p-4 sm:space-y-6 sm:p-6 lg:px-8">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p
              className={`mt-1 text-sm ${isDarkMode ? "text-slate-300" : "text-slate-700"}`}
            >
              Complete the daily forecast sequence from analysis through the
              48-hour outlook.
            </p>
          </div>
          <div className="flex flex-col gap-2 sm:flex-row">
            ...
            <button
              ...
              disabled={loading}
              className={`group relative inline-flex min-h-11 items-center justify-center gap-2 overflow-hidden rounded-xl border px-4 text-sm font-bold backdrop-blur-2xl transition duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300 disabled:cursor-not-allowed disabled:opacity-50 ${isDarkMode ? "border-white/20 bg-white/[0.07] text-white shadow-[inset_0_1px_0_rgba(255,255,255,0.18),0_10px_28px_rgba(1,15,35,0.28)] hover:border-cyan-200/40 hover:bg-white/[0.11]" : "border-white/85 bg-white/62 text-slate-900 shadow-[inset_0_1px_0_rgba(255,255,255,0.94),0_10px_28px_rgba(15,65,90,0.14)] hover:border-cyan-200 hover:bg-white/82"}`}
            >
