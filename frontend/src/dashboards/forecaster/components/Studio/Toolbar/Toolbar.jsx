...
const getTheme = (isDarkMode) => ({
  ...
  disabled: isDarkMode
    ? 'border-cyan-100/14 bg-white/[0.075] text-cyan-50/90 cursor-not-allowed'
  ...
  subtle: isDarkMode ? 'text-white/90' : 'text-slate-700',
});
...
const TrayButton = ({ active, activeClassName, disabled = false, icon, label, onClick, theme }) => (
  ...
  >
    ...
    <span className="truncate text-[11px] font-black leading-tight tracking-[0.035em] [text-shadow:0_1px_2px_rgba(0,0,0,0.95),0_0_8px_rgba(14,165,233,0.35)]">{label}</span>
  </button>
);
  ...
  return (
    <>
      ...
      <FloatingShell isDragging={isDragging} position={position} width={dockWidth} theme={theme}>
        {isCollapsed ? (
          <div className="relative z-10 flex h-12 items-center gap-1.5"><button type="button" aria-label="Open drawing tools" title="Open drawing tools" onClick={handleToggleCollapse} className="flex min-w-0 flex-1 items-center gap-2 rounded-[18px] px-1.5 py-1 text-xs font-black tracking-[0.02em]"><span className={cn('flex h-8 w-8 shrink-0 items-center justify-center rounded-2xl', theme.iconRail)}><TbTools size={16} aria-hidden="true" /></span><span className="min-w-0 flex-1 truncate text-left [text-shadow:0_1px_2px_rgba(0,0,0,0.95),0_0_8px_rgba(14,165,233,0.35)]">Draw Tools</span><ChevronUp size={14} className={theme.subtle} aria-hidden="true" /></button></div>
        ) : (
