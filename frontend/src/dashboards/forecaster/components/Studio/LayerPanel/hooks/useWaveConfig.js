...
export const useWaveConfig = ({ mapRef, isDarkMode }) => {
  ...
  const ww3ForecastHours = useMemo(
    () => getWW3ForecastHours(chartType),
    [chartType],
  );
  ...
  const [ww3Selection, setWW3Selection] = useState(() => ({
    packageKey: null,
    forecastHour: defaultWW3ForecastHour,
  }));
  const ww3ForecastHour =
    ww3Selection.packageKey === packageKey
      ? ww3Selection.forecastHour
      : defaultWW3ForecastHour;
  const ecwamForecastHours = useMemo(
    () => getECWAMForecastHours(chartType),
    [chartType],
  );
  ...
  const visibleEcwamFrameState =
    ecwamFrameState.packageKey === packageKey
      ? ecwamFrameState
      : IDLE_ECWAM_FRAME;
    ...
    async (nextValue) => {
      ...
      if (!ecwamForecastHours.includes(nextHour)) {
        const result = {
          state: "invalid",
          message:
            "This forecast hour is outside the ECWAM window for the current chart.",
        };
      ...
      if (!packageDate) {
        const result = {
          state: "invalid",
          message: "The forecast package date is not available.",
        };
      ...
      try {
        ...
        if (requestId !== ecwamRequestRef.current)
          return { state: "cancelled" };
        const result = {
          state: "failed",
          requestedHour: nextHour,
          message:
            error?.message || "Unable to load the requested ECWAM frame.",
        };
        setEcwamFrameState({ packageKey, ...result });
        return result;
      }
  ...
  const setWW3ForecastHour = useCallback(
    (nextValue) => {
      const nextHour = Number(nextValue);
      if (!ww3ForecastHours.includes(nextHour)) {
        return {
          state: "invalid",
          message:
            "This forecast hour is outside the WW3 window for the current chart.",
        };
      }

      setWW3Selection({ packageKey, forecastHour: nextHour });
      return { state: "ready", forecastHour: nextHour };
    },
    [packageKey, ww3ForecastHours],
  );

  const stepWW3ForecastHour = useCallback(
    (direction) => {
      const currentIndex = ww3ForecastHours.indexOf(ww3ForecastHour);
      if (currentIndex < 0) return { state: "invalid" };
      const nextIndex = Math.min(
        ww3ForecastHours.length - 1,
        Math.max(0, currentIndex + Math.sign(direction)),
      );
      if (nextIndex === currentIndex) {
        return { state: "ready", forecastHour: ww3ForecastHour };
      }
      return setWW3ForecastHour(ww3ForecastHours[nextIndex]);
    },
    [setWW3ForecastHour, ww3ForecastHour, ww3ForecastHours],
  );
  ...
  const toggleWaveLayer = useCallback(() => {
    ...
    addWaveLayer(currentMap, isDarkMode, next.models).then(() =>
      applyLayers(next),
    );
  ...
  const toggleWaveModel = useCallback(
    (model) => {
      ...
      if (isAdding && next.enabled) {
        addWaveLayer(currentMap, isDarkMode, [normalized]).then(() =>
          applyLayers(next),
        );
        return;
      }
  ...
  const waveConfigView = useMemo(
    () => ({
      ...
      ww3Frame: {
        ...
        maxHour: ww3ForecastHours[ww3ForecastHours.length - 1],
      },
      setWW3ForecastHour,
      stepWW3ForecastHour,
      ecwamFrame: {
