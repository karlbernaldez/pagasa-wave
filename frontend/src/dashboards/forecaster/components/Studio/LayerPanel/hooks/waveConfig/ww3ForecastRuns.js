...
const chartWindowKey = (chartType) => {
  ...
  const hourMatch = normalized.match(
    /(?:forecast\s*)?(24|36|48)\s*(?:h|hr|hour)?/,
  );
  return hourMatch ? `${hourMatch[1]}h` : "analysis";
};

export const getWW3ForecastHours = (chartType) =>
  WW3_CHART_WINDOWS[chartWindowKey(chartType)];

export const getECWAMForecastHours = (chartType) =>
  ECWAM_CHART_WINDOWS[chartWindowKey(chartType)];
...
const resolveOffset = (chartType) => {
  ...
  return (
    WW3_FORECAST_OFFSETS[`forecast ${hourMatch[1]}h`] ?? DEFAULT_WW3_OFFSET
  );
};
...
const resolveECWAMOffset = (chartType) => {
  ...
  return (
    ECWAM_FORECAST_OFFSETS[`forecast ${hourMatch[1]}h`] ?? DEFAULT_ECWAM_OFFSET
  );
};
...
const normalizeForecastHour = (forecastHour) => {
  ...
  return Number.isInteger(hour) && hour >= 0 && hour <= 60 && hour % 3 === 0
    ? hour
    : null;
};
...
export const resolveWW3ForecastRun = ({
  ...
  if (explicitHour !== null) {
    validTime = new Date(
      analysisTime.getTime() + explicitHour * 60 * 60 * 1000,
    );
...
export const resolveECWAMForecastRun = ({
  ...
  if (explicitHour !== null) {
    validTime = new Date(
      packageBaseDate.getTime() + explicitHour * 60 * 60 * 1000,
    );
    resolvedForecastHour = explicitHour;
  } else {
