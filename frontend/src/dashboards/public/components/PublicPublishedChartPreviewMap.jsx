...
function isFront(feature) {
  ...
  const label = `${props.name || ''} ${props.title || ''} ${feature?.name || ''}`.toLowerCase();
  return Boolean(props.isFront || props.frontType || label.includes('front'));
}
...
function buildLineLabels(features) {
  const labels = [];
...
function splitFeatureCollection(featureCollection) {
  const fronts = [];
  const regular = [];
  ...
  return {
    ...
    labels: buildLineLabels(regular),
  };
}
...
function syncAnnotations(map, featureCollection) {
  const { regular, fronts, labels } = splitFeatureCollection(featureCollection);
...
function PublicPublishedChartPreviewMap({ projectId, initialRaster, isDarkMode = false, className = '', height = 288, onClick }) {
  ...
  useEffect(() => {
    ...
    map.fitBounds(DEFAULT_BOUNDS, { padding: 16, maxZoom: 6, duration: 0 });
  }, [featureCollection, hasFeatures, raster]);
