...
// Helper: detect if a logo value is an image URL vs emoji/text
const isImageUrl = (val) => typeof val === 'string' && (val.startsWith('http') || val.startsWith('/'));

// Uniform logo renderer — always 48×48 inside a gradient rounded square
const PartnerLogo = ({ logo, acronym, color }) => (
  <div
    className={`w-14 h-14 flex-shrink-0 flex items-center justify-center rounded-xl bg-gradient-to-br ${color} shadow-lg`}
  >
    {isImageUrl(logo) ? (
      <img
        src={logo}
      ...
      <span className="text-2xl leading-none select-none">{logo}</span>
    )}
  </div>
);
...
const PartnersCollaborationSection = ({ isDark }) => {
  ...
  const partners = [
    {
      ...
      logo: 'https://upload.wikimedia.org/wikipedia/commons/9/9a/DOST_seal.svg',
    ...
    },
    {
      ...
      role: 'Research Funding & Oversight',
      description: 'Council under DOST that funds and manages R&D programs in industry, energy, and emerging technologies.',
      collaboration: ['Research funding', 'Program management', 'Innovation support'],
      logo: '/pcieerd-icon.webp',
      color: 'from-white to-gray-200',
      featured: true
    },
    {
    ...
    },
    {
      ...
      acronym: 'CWA',
      role: 'Typhoon Forecasting Partner',
      description: "Taiwan's national weather authority and a leading center for typhoon research and forecasting in the Western Pacific region.",
      collaboration: ['Typhoon track data sharing', 'Numerical weather prediction', 'Joint forecast coordination'],
      logo: 'https://www.cwa.gov.tw/V8/assets/img/logoBlue.svg',
      color: 'from-white to-gray-200',
      featured: true
    },
  ];
  ...
  return (
    <section className={`relative py-20 lg:py-32 overflow-hidden transition-all duration-700 ${isDark
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-16 lg:mb-20">
          <div className={`inline-flex items-center gap-2.5 px-5 py-2.5 rounded-full text-sm font-semibold mb-8 backdrop-blur-sm transition-all duration-300 hover:scale-[1.02] ${isDark
          ...
          <h2 className={`text-4xl sm:text-5xl lg:text-6xl font-black leading-tight mb-6 tracking-tight transition-colors duration-700 ${isDark ? 'text-white' : 'text-slate-900'
            }`}>
            Our{' '}
          ...
          <p className={`text-lg sm:text-xl max-w-4xl mx-auto leading-relaxed transition-colors duration-700 ${isDark ? 'text-slate-300' : 'text-slate-600'
            }`}>
        ...
        <div className={`p-8 lg:p-10 rounded-2xl backdrop-blur-sm border mb-16 lg:mb-20 ${isDark
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-center">
            <div className="lg:col-span-2">
              <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold mb-4 ${isDark
              ...
              <h3 className={`text-2xl lg:text-3xl font-black mb-2 tracking-tight transition-colors duration-700 ${isDark ? 'text-white' : 'text-slate-900'
                }`}>
                {projectInfo.title}
              </h3>
              <p className={`text-lg mb-4 transition-colors duration-700 ${isDark ? 'text-blue-300' : 'text-blue-700'
                }`}>
            ...
            <div className={`p-6 rounded-xl ${isDark ? 'bg-slate-800/50' : 'bg-white/70'
              }`}>
              ...
              <ul className="space-y-2">
                {projectInfo.objectives.map((obj, index) => (
                  <li key={index} className={`flex items-start gap-2 text-xs ${isDark ? 'text-slate-400' : 'text-slate-600'
                    }`}>
        ...
        <div className="flex flex-wrap justify-center gap-3 mb-12">
          {categories.map((category) => (
            <button
              ...
              className={`group flex items-center gap-2 px-5 py-2.5 rounded-xl font-semibold text-sm transition-all duration-300 hover:scale-[1.02] ${activeCategory === category.id
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8 mb-20">
          {filteredPartners.map((partner, index) => (
            ...
            >
              <div className={`relative h-full p-8 rounded-2xl backdrop-blur-sm border transition-all duration-300 hover:scale-[1.02] overflow-hidden ${partner.featured
                ? isDark
                ...
                <div className="relative">
                  ...
                  <div className="flex items-start gap-5 mb-6">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className={`text-sm font-bold mb-1 ${isDark ? 'text-blue-400' : 'text-blue-700'
                        }`}>
                        {partner.acronym}
                      </div>
                      <h3 className={`text-xl lg:text-2xl font-bold mb-2 leading-tight transition-colors duration-300 ${isDark ? 'text-white group-hover:text-blue-300' : 'text-slate-900 group-hover:text-blue-700'
                        }`}>
                        {partner.name}
                      </h3>
                      <div className={`inline-block px-3 py-1 rounded-full text-xs font-semibold ${isDark
                  ...
                  {/* Collaboration Areas */}
                  <div>
                    <h4 className={`text-sm font-bold mb-3 ${isDark ? 'text-slate-400' : 'text-slate-700'
                      }`}>
                    ...
                    <div className="space-y-2">
                      {partner.collaboration.map((item, itemIndex) => (
                        <div
                          key={itemIndex}
                          className={`flex items-center gap-3 p-3 rounded-xl transition-all duration-300 hover:scale-[1.02] ${isDark
        ...
        {/* Statistics */}
        <div className={`p-8 lg:p-10 rounded-2xl backdrop-blur-sm border mb-16 ${isDark
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
            {stats.map((stat, index) => (
              <div
                key={index}
                className={`text-center group transition-all duration-500 hover:scale-110 p-6 rounded-xl ${isDark ? 'hover:bg-slate-800/30' : 'hover:bg-white/70'
                  }`}
              >
                ...
                <div className={`text-lg font-bold mb-1 transition-colors duration-300 ${isDark ? 'text-white' : 'text-slate-900'
                  }`}>
                  {stat.label}
                </div>
        ...
        {/* CTA Section */}
        <div className={`p-8 lg:p-10 rounded-2xl backdrop-blur-sm border text-center transition-all duration-300 hover:scale-[1.01] ${isDark
          ...
          <Building2 className={`w-16 h-16 mx-auto mb-6 ${isDark ? 'text-blue-400' : 'text-blue-600'
            }`} />
          <h3 className={`text-3xl lg:text-4xl font-bold mb-4 transition-colors duration-700 ${isDark ? 'text-white' : 'text-slate-900'
            }`}>
            Interested in Partnering with Us?
          </h3>
          <p className={`text-lg mb-8 max-w-2xl mx-auto transition-colors duration-700 ${isDark ? 'text-slate-300' : 'text-slate-600'
            }`}>
