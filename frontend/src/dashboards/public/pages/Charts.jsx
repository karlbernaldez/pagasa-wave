...
const CHART_STYLES = [
  {
    ...
    icon: Wind,
    description: 'Wave raster plus published annotations.',
    color: '#2563eb',
  },
  {
    id: 'wave-only',
    ...
    icon: Waves,
    description: 'Annotation-focused chart without raster.',
    color: '#0891b2',
  },
  {
    ...
    icon: Eye,
    description: 'Higher contrast labels and viewing mode.',
    color: '#059669',
  },
];
...
function LiquidBackdrop({ isDark }) {
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden" aria-hidden="true">
      <div
        className={`absolute -left-40 top-20 h-[520px] w-[520px] rounded-full blur-3xl ${isDark ? 'bg-cyan-500/10' : 'bg-blue-300/25'}`}
      />
      <div
        className={`absolute -right-36 top-72 h-[500px] w-[500px] rounded-full blur-3xl ${isDark ? 'bg-blue-700/10' : 'bg-cyan-200/30'}`}
      />
    </div>
  );
}
...
function StateNotice({ isDark, tone = 'slate', title, children, action }) {
  ...
  return (
    <div
      className={`mx-auto max-w-2xl rounded-3xl border p-6 text-center backdrop-blur-xl ${toneClass}`}
    >
...
function CompletenessBadge({ completeness, isDark }) {
  if (completeness.isComplete) {
    return (
      <span
        className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-xs font-black ${isDark ? 'border-emerald-400/20 bg-emerald-400/10 text-emerald-100' : 'border-emerald-200 bg-emerald-50 text-emerald-700'}`}
      >
  ...
  if (completeness.isEmpty) {
    return (
      <span
        className={`rounded-full border px-3 py-1 text-xs font-black ${isDark ? 'border-white/10 bg-white/5 text-slate-400' : 'border-slate-200 bg-white/70 text-slate-500'}`}
      >
        No charts
      </span>
    );
  ...
  return (
    <span
      className={`rounded-full border px-3 py-1 text-xs font-black ${isDark ? 'border-amber-400/20 bg-amber-400/10 text-amber-100' : 'border-amber-200 bg-amber-50 text-amber-700'}`}
    >
      Incomplete
    </span>
  );
  ...
  return (
    <section
      className={glassPanel(
        isDark,
        'sticky top-[4.75rem] z-30 mt-4 grid w-full gap-3 p-3 lg:grid-cols-[minmax(0,1fr)_420px]'
      )}
    >
      <label
        className={`flex min-w-0 items-center gap-3 rounded-2xl border px-4 py-3 ${isDark ? 'border-white/10 bg-slate-950/55' : 'border-slate-200/80 bg-white/70'}`}
      >
      ...
      <div
        className={`grid grid-cols-3 rounded-2xl border p-1 ${isDark ? 'border-white/10 bg-slate-950/55' : 'border-slate-200/80 bg-white/70'}`}
        aria-label="Chart display style"
      >
        {CHART_STYLES.map((style) => {
          ...
          return (
            <button
              ...
              className={`flex items-center justify-center gap-2 rounded-xl px-3 py-2.5 text-xs font-black transition-all focus:outline-none focus:ring-2 focus:ring-blue-400/60 ${active ? (isDark ? 'bg-cyan-300/15 text-cyan-50 shadow-lg shadow-cyan-950/20' : 'solid-blue shadow-sm') : isDark ? 'text-slate-400 hover:bg-white/5 hover:text-slate-100' : 'text-slate-500 hover:bg-white/80 hover:text-slate-900'}`}
            >
              <Icon size={15} />
...
function ChartSlotCard({ slot, chart, activeStyle, isDark, onOpen, showStaffInfo }) {
  ...
  const description = getPublicChartCardDescription({
    ...
    hasChart,
    formatDate,
    getPersonName: publicNameFormatter,
  });
  ...
  return (
    <article
      className={`${glassPanel(isDark, 'group relative flex min-h-[350px] flex-col overflow-hidden rounded-[1.65rem] transition duration-300 hover:-translate-y-1')} ${isDark ? 'hover:border-cyan-300/30' : 'hover:border-blue-200'}`}
    >
      <div
        className="absolute inset-x-0 top-0 h-px"
        style={{ background: `linear-gradient(90deg, transparent, ${style.color}88, transparent)` }}
        aria-hidden="true"
      />
      <div className="relative h-44 overflow-hidden text-left xl:h-48">
        ...
        <div className="absolute left-4 top-4 z-20">
          <span
            className="rounded-xl px-3 py-1.5 text-[11px] font-black uppercase tracking-widest text-white shadow-lg shadow-black/20"
        ...
        {hasChart ? (
          <div className="pointer-events-none absolute inset-0 z-20 flex items-center justify-center bg-slate-950/25 opacity-0 transition-opacity duration-200 group-hover:opacity-100">
            <span className="flex items-center gap-2 rounded-full bg-slate-950/55 px-5 py-3 text-sm font-black text-white backdrop-blur-sm">
              <ArrowRight size={15} /> Open chart
            </span>
          </div>
        ) : (
      ...
      <div className="flex flex-1 flex-col justify-between p-4">
        <div>
          <h2
            className={`text-base font-black leading-tight tracking-tight ${isDark ? 'text-white' : 'text-slate-900'}`}
          >
            {title}
          </h2>
          <p
            className={`mt-2 text-xs font-semibold ${isDark ? 'text-slate-400' : 'text-slate-500'}`}
          >
            {metaText}
          </p>
          <p
            className={`mt-3 line-clamp-2 text-sm font-medium leading-relaxed ${isDark ? 'text-slate-400' : 'text-slate-600'}`}
          >
            {description}
          </p>
        </div>
        <div
          className={`mt-4 flex items-center justify-between border-t pt-3 ${isDark ? 'border-white/10' : 'border-slate-100'}`}
        >
          <span
            className={`rounded-full border px-3 py-1 text-xs font-black ${isDark ? 'border-white/10 bg-white/5 text-slate-300' : 'border-slate-200 bg-white/70 text-slate-600'}`}
          >
            {style.label}
          </span>
      ...
      <div className="flex gap-3 overflow-x-auto pb-1">
        {grouped.map((item) => {
          ...
          return (
            ...
            >
              <p className="text-sm font-black">{formatDate(item.dateKey)}</p>
              <p
                className={`mt-2 text-xs font-black ${isDark ? 'text-slate-500' : 'text-slate-400'}`}
              >
                {item.availableCount}/4 charts
              </p>
            </button>
          );
        })}
...
export default function Charts() {
  ...
  useEffect(() => {
    document.title = 'Wavelab | Published Wave Charts';
  }, []);
    ...
    fetchPublicPublishedCharts({
      ...
      .then((data) =>
        setState({
          ...
          error: '',
          projects: filterProjectsToPublicChartWindow(
            data?.projects || [],
            getPublicChartTenDayWindow()
          ),
        })
      ...
      .catch((error) => {
        if (error?.name !== 'AbortError') {
          setState({
            requestKey: chartRequestKey,
            error: error?.message || 'Failed to load public charts.',
            projects: [],
          });
        }
      });
  ...
  const selectedDateAvailable =
    selectedDate && historyGroups.some((item) => item.dateKey === selectedDate);
  ...
  const openChart = useCallback(
    (chart) => {
      if (chart?._id) navigate(`/charts/${chart._id}`);
    },
    [navigate]
  );
    ...
    >
      ...
      <div className="relative z-10 mx-auto flex max-w-7xl flex-col gap-6">
        <section
          className={glassPanel(
            isDark,
            'grid gap-5 p-5 sm:p-7 lg:grid-cols-[minmax(0,1fr)_330px_auto] lg:items-center'
          )}
        >
          <div>
            <p className="text-xs font-black uppercase tracking-[0.2em] text-blue-700">
              Published forecast archive
            </p>
            <h1
              className={`mt-3 text-4xl font-black leading-tight tracking-tight sm:text-5xl ${isDark ? 'text-white' : 'text-slate-950'}`}
            >
              Published Wave Charts
            </h1>
            <p
              className={`mt-3 max-w-2xl text-base font-semibold leading-relaxed ${isDark ? 'text-slate-300' : 'text-slate-700'}`}
            >
              Browse official published wave chart sets by date, chart style, and release package.
            </p>
          </div>
          <div
            className={cx(
              'rounded-[1.45rem] border p-4',
              isDark ? 'border-white/10 bg-white/[0.055]' : 'border-white/60 bg-white/55'
            )}
          >
            ...
            <h2
              className={`mt-3 flex items-center gap-2 text-3xl font-black ${isDark ? 'text-white' : 'text-slate-950'}`}
            >
              <CalendarDays size={27} className="text-blue-700" />{' '}
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              ...
              <span
                className={cx(
                  ...
                  isDark ? 'bg-blue-400/10 text-cyan-100' : 'bg-blue-50 text-blue-800'
                )}
              >
                {forecastPeriodLabel}
              </span>
        ...
        {chartListLoading && (
          <section className="space-y-4">
            <StateNotice isDark={isDark} title="Loading latest chart set">
              Fetching published charts and preparing the public chart cards.
            </StateNotice>
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              {PUBLIC_CHART_SLOTS.map((slot) => (
                <div
                  key={slot.chartType}
                  className={glassPanel(isDark, 'h-[350px] animate-pulse')}
                />
              ))}
            </div>
          </section>
        ...
        {!chartListLoading && !chartListError && !recentProjects.length && (
          <StateNotice
            isDark={isDark}
            title={query.trim() ? 'No charts match this search' : 'No published wave charts found'}
          >
            {query.trim()
              ? 'Clear the search to view recent published outputs.'
              : 'Published charts will appear here once Admin publishes approved outputs.'}
          </StateNotice>
        )}
          ...
          <>
            <section className="space-y-4">
              <div
                className={glassPanel(
                  isDark,
                  'grid gap-4 p-4 sm:p-5 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center'
                )}
              >
                <div>
                  <p className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.2em] text-blue-700">
                    <CalendarDays size={15} /> Current chart set summary
                  </p>
                  <h2
                  ...
                  <p
                    className={`mt-2 max-w-2xl text-xs font-semibold leading-relaxed ${isDark ? 'text-slate-400' : 'text-slate-500'}`}
                  >
                    {pdfStatusText}
                  </p>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                  <CompletenessBadge completeness={completeness} isDark={isDark} />
                  <div
                    className={`rounded-2xl border px-4 py-3 text-sm font-black ${isDark ? 'border-white/10 bg-white/5 text-slate-300' : 'border-slate-200 bg-white/80 text-slate-600'}`}
                  >
        ...
        <div
          className={glassPanel(
            isDark,
            'flex flex-col gap-3 p-4 text-xs font-semibold sm:flex-row sm:items-center sm:justify-between'
          )}
        >
          <p className={isDark ? 'text-slate-400' : 'text-slate-600'}>
            <span className="font-black text-blue-700">Important Notice:</span> Wave forecast charts
            are guidance products. Always check official marine advisories and local conditions.
          </p>
          <p className={`tabular-nums ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>
