...
const Header = () => {
  ...
  return (
    <>
                ...
                {item.hasDropdown ? (
                  ...
                  >
                    ...
                    >
                      ...
                      <ChevronDown
                        size={14}
                        className={`transition-transform duration-200 ${isChartDropdownOpen ? 'rotate-180' : 'rotate-0'
              ...
              <div ref={dropdownRef} className="hidden md:block relative">
                ...
                >
                  ...
                  <div className="hidden lg:block text-left min-w-0">
                    <div className={`text-sm font-medium leading-tight whitespace-nowrap overflow-hidden text-ellipsis max-w-[120px] ${isDarkMode ? 'text-gray-50' : 'text-gray-900'
                      }`}>
                      {currentUser?.username || 'Loading...'}
                    </div>
                    <div className={`text-xs leading-tight whitespace-nowrap overflow-hidden text-ellipsis max-w-[120px] ${isDarkMode ? 'text-gray-400' : 'text-gray-500'
                      }`}>
                  ...
                  <ChevronDown
                    size={16}
                    className={`flex-shrink-0 transition-transform duration-200 ${isUserDropdownOpen ? 'rotate-180' : 'rotate-0'
                ...
                {/* User Dropdown Menu */}
                {isUserDropdownOpen && (
                  <div className={`absolute top-full right-0 mt-2 min-w-[240px] z-50 overflow-hidden rounded-lg border shadow-xl ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'
                    }`}>
                    <div className={`p-4 border-b ${isDarkMode ? 'border-gray-700 bg-gray-900' : 'border-gray-200 bg-gray-50'
                      }`}>
                      <div className={`text-sm font-semibold mb-0.5 break-words ${isDarkMode ? 'text-gray-50' : 'text-gray-900'
                        }`}>
                        {currentUser?.username || 'User'}
                      </div>
                      <div className={`text-xs break-words ${isDarkMode ? 'text-gray-400' : 'text-gray-500'
                        }`}>
                        {currentUser?.email || 'user@example.com'}
                      </div>
                      <div className={`text-[11px] uppercase tracking-wide mt-1 font-medium ${isDarkMode ? 'text-gray-500' : 'text-gray-400'
                        }`}>
                    ...
                    <button
                      ...
                      className={`w-full px-4 py-3 border-none bg-transparent cursor-pointer flex items-center gap-3 text-sm transition-colors duration-150 text-left ${isDarkMode ? 'text-gray-50 hover:bg-gray-700' : 'text-gray-900 hover:bg-gray-100'
                        }`}
                    >
                      <User size={16} />
                      View Profile
      ...
      {isMobileMenuOpen && (
        <div
          className={`fixed top-[70px] left-0 w-full h-[calc(100%-70px)] backdrop-blur-lg z-40 px-4 py-8 overflow-y-auto flex flex-col gap-4 ${isDarkMode ? 'bg-slate-900/95' : 'bg-white/95'
            }`}
        >
          {navItems.map((item, index) => (
            <div key={index}>
