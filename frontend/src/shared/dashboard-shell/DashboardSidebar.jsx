...
const DashboardSidebar = ({
  ...
  const renderSoonBadge = (disabled) =>
    !isSidebarCollapsed && disabled ? (
      <span
        className={`rounded-md px-1.5 py-0.5 text-[10px] font-black uppercase tracking-[0.08em] ${
          isDarkMode
            ? "bg-white/[0.06] text-slate-400/80"
      ...
      <span className="flex min-w-0 items-center gap-3">
        ...
        <span
          className={`grid h-9 w-9 shrink-0 place-items-center rounded-lg ${
            isActive
              ? isDarkMode
                ? "bg-cyan-300/12 text-cyan-200 shadow-inner shadow-cyan-300/10"
                : "bg-cyan-50/80 text-cyan-700 shadow-inner shadow-white"
              : "text-current"
          }`}
        >
          <Icon size={18} />
    ...
    return (
      <div key={item.id ?? item.path ?? item.label}>
        ...
        {!isSidebarCollapsed && hasChildren && isExpanded && (
          <div
            className={`ml-7 mt-1 space-y-1 border-l pl-4 ${
              isDarkMode ? "border-white/10" : "border-white/80"
            }`}
          >
            {item.children.map((child) => {
              const childActive =
                child.isActive?.(activeId) ?? activeId === child.id;
  ...
  const renderNavGroup = (group, index) => (
    ...
    >
      {!isSidebarCollapsed && (
        <div
          className={`mb-2 px-3 text-[10px] font-black uppercase tracking-[0.16em] ${getMutedText(isDarkMode)}`}
        >
          {group.label}
        </div>
      )}
  ...
  return (
    <>
      ...
      >
        <div
          className={`border-b px-4 py-4 ${isDarkMode ? "border-white/10" : "border-white/70"}`}
        >
            ...
            <div className="flex min-w-0 items-center gap-3">
              <span
                className={`grid h-11 w-11 shrink-0 place-items-center rounded-xl border ${
                  isDarkMode
                    ? "border-white/10 bg-white/[0.06] shadow-inner shadow-white/[0.04]"
                    : "border-white/80 bg-white/70 shadow-sm shadow-slate-200/70"
                }`}
              >
                {backgroundVariant === "ocean" ? (
                  <Waves
              ...
              {!isSidebarCollapsed && (
                <div className="min-w-0">
                  <h1
                    className={`truncate text-base font-black tracking-tight ${isDarkMode ? "text-white" : "text-slate-950"}`}
                  >
                    {title}
                  </h1>
                  <p
                    className={`truncate text-xs font-bold ${isDarkMode ? "text-slate-300/70" : "text-slate-600"}`}
                  >
                    {label}
                  </p>
                </div>
        ...
        <div
          className={`border-t p-3 ${isDarkMode ? "border-white/10" : "border-white/70"}`}
        >
          <button
            type="button"
            title="Help & Support"
          ...
          {!isSidebarCollapsed && (
            <div className="mt-3 flex items-start gap-3 px-3 pb-1">
              ...
              <div className="min-w-0">
                ...
                <p
                  className={`mt-0.5 text-[9px] font-semibold leading-snug ${getMutedText(isDarkMode)}`}
                >
                  {footerText}
                </p>
              </div>
