        ...
        >
          ...
          <div className="max-h-[min(420px,calc(100vh-150px))] overflow-y-auto">
            {isLoading ? (
              ...
              <div className="p-5 text-sm font-semibold text-red-500">{error}</div>
            ) : notifications.length === 0 ? (
              <div className={`p-6 text-center text-sm font-semibold ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>
                ...
                return (
                  <button
                    ...
                    className={`w-full border-b px-4 py-3 text-left transition last:border-b-0 ${
                      isDarkMode
                        ? `border-white/10 ${unread ? 'bg-cyan-500/10 hover:bg-cyan-500/15' : 'hover:bg-white/5'}`
                        : `border-slate-100 ${unread ? 'bg-blue-50 hover:bg-blue-100/70' : 'hover:bg-slate-50'}`
                    }`}
                  >
                    <div className="flex items-start gap-3">
                      <span className={`mt-1 h-2 w-2 shrink-0 rounded-full ${unread ? 'bg-blue-500' : isDarkMode ? 'bg-slate-700' : 'bg-slate-200'}`} />
                      <span className="min-w-0 flex-1">
                        <span className="flex items-start justify-between gap-2">
                          <span className="text-sm font-black leading-snug">{notification.title}</span>
                          {canOpen && <ExternalLink size={13} className={isDarkMode ? 'text-slate-500' : 'text-slate-400'} />}
                        </span>
                        <span className={`mt-1 block text-xs font-semibold leading-relaxed ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>
                          {notification.message}
                        </span>
                        <span className={`mt-2 block text-[11px] font-bold ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>
