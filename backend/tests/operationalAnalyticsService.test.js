...
test('user contributions use meaningful forecast workflow events and exclude styling or movement metrics', async () => {
      ...
      return [
        {
          ...
          byDay: [
            ...
            { _id: { date: '2026-09-10', action: 'chart_completion_updated' }, count: 3 },
            { _id: { date: '2026-09-11', action: 'chart_completion_updated' }, count: 2 },
          ],
        },
      ];
    },
  ...
  assert.equal(CONTRIBUTION_ACTIONS.some((action) => /style|move|annotation/i.test(action)), false);
...
test('published chart view analytics are date-bounded and include only aggregate public-safe chart fields', async () => {
  let capturedPipeline = null;
  ...
  const lookup = capturedPipeline[1].$facet.topCharts.find((stage) => stage.$lookup)?.$lookup;
