...
test("disconnects active Socket.IO transports before closing HTTP", () => {
  ...
  assert.equal(
    harness.events.some(
      (event) => event[0] === "info" && event[1] === "WaveLab API stopped",
    ),
    true,
  );
});
...
test("keeps the bounded forced-shutdown failure path", () => {
  ...
  assert.equal(
    harness.events.some(
      (event) =>
        event[0] === "error" && event[1] === "WaveLab API shutdown timed out",
    ),
    true,
  );
});
...
test("reports a synchronous Socket.IO disconnect failure", () => {
  ...
  const shutdown = createShutdownHandler({
    ...
    httpServer: {
      close() {
        assert.fail(
          "HTTP close must not run after Socket.IO disconnect throws",
        );
      },
    },
    logger: {
