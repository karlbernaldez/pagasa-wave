...
function getPreviousIncompleteChartType(chartCompletion = [], chartType) {
  ...
  return (
    REQUIRED_FORECAST_CHART_TYPES.slice(0, chartIndex).find((previousChartType) => {
...
function getParticipants(chart = {}) {
  const participants = Array.isArray(chart.participants) ? [...chart.participants] : [];
  getActiveEditors(chart).forEach((editor) => {
    if (!participants.some((participant) => isSameId(participant.user, editor.user)))
      participants.push({
        user: editor.user,
        firstJoinedAt: editor.startedAt || new Date(),
        lastJoinedAt: editor.startedAt || new Date(),
      });
  });
  return participants;
}
function getReadyEditors(chart = {}) {
  return Array.isArray(chart.readyEditors) ? [...chart.readyEditors] : [];
}
...
function applyLegacyClaimFromActiveEditors(chart, activeEditors) {
  const firstEditor = activeEditors[0];
...
function ensureParticipant(chart, userId) {
  const now = new Date();
  if (!Array.isArray(chart.participants)) chart.participants = [];
  const existing = chart.participants.find((participant) => isSameId(participant.user, userId));
  if (existing) existing.lastJoinedAt = now;
  else chart.participants.push({ user: userId, firstJoinedAt: now, lastJoinedAt: now });
}
function isUserReady(chart, userId) {
  return getReadyEditors(chart).some((vote) => isSameId(vote.user, userId));
}
function removeReadyVote(chart, userId) {
  chart.readyEditors = getReadyEditors(chart).filter((vote) => !isSameId(vote.user, userId));
}
...
function serializePackage(forecastPackage) {
  ...
  return { ...plain, completion: getPackageCompletion(plain.chartCompletion || []) };
}
function serializeChartContext(forecastPackage, chart, user) {
  ...
  const participantLabels = participants
    .map((participant) => getDisplayName(participant.user))
    .filter(Boolean);
  const readyEditorLabels = readyEditors.map((vote) => getDisplayName(vote.user)).filter(Boolean);
  const activeEditorLabels = activeEditors
  ...
  const readyCount = participants.filter((participant) =>
    readyUserIds.has(String(participant.user?._id || participant.user))
  ).length;
  ...
  return {
    ...
    claim: {
      ...
      activeEditorLabels,
      canClaim:
        editable && !isReady && !blockingChartType && !activeEditorCurrentUser && !currentUserReady,
...
function updateChartCompletionState(forecastPackage, chart, isComplete, user) {
  ...
  const blockingChartType = isComplete
    ? getPreviousIncompleteChartType(forecastPackage.chartCompletion || [], chart.chartType)
    : null;
  if (blockingChartType)
    throwError(
      `${getForecastChartLabel(blockingChartType)} must be certified before ${getForecastChartLabel(chart.chartType)}`,
      400
    );
  let comment;
  if (isComplete) {
    if (!activeEditorCurrentUser)
      throwError(
        `${getForecastChartLabel(chart.chartType)} must be opened for editing by you before you can mark yourself ready`,
        409
      );
    ...
    const readyUserIds = new Set(
      getReadyEditors(chart).map((vote) => String(vote.user?._id || vote.user))
    );
    ...
    if (!allParticipantsReady) {
      const remainingCount = participants.filter(
        (participant) => !readyUserIds.has(String(participant.user?._id || participant.user))
      ).length;
    ...
    comment =
      resetCount > 1
        ? `${getForecastChartLabel(chart.chartType)} and downstream charts marked incomplete`
        : `${getForecastChartLabel(chart.chartType)} marked incomplete`;
  }
...
async function getPackageByProject(projectId) {
...
async function joinChartIfAllowedByProject(
  projectId,
  user,
...
export const joinForecastPackageChartEditingByProject = asyncHandler(async (req, res) => {
  ...
  await joinChartIfAllowedByProject(req.params.projectId, req.user, {
    emit: true,
    audit: true,
    strict: true,
  });
