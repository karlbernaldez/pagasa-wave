...
async function ensureFeatureMutationAllowed(sourceId, user) {
  ...
  const isOwner = isSameId(getFeatureOwner(feature), user.id);
...
export const createFeature = asyncHandler(async (req, res) => {
  ...
  if (result.upsertedCount > 0) {
    emitAnnotationUpdate(projectId, 'annotation_created', sourceId);
  }
...
// REQUEST OWNER ACTION FOR SHARED ANNOTATION
export const requestFeatureChange = asyncHandler(async (req, res) => {
  ...
  if ((normalizedType === 'rename' || normalizedType === 'label_change') && !normalizedRequestedName) {
