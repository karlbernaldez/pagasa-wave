...
import { buildChatRequest, resolveChatTier } from '../chat/services/chatService.js';
...
export const proxyChatCompletion = asyncHandler(async (req, res) => {
  ...
  const inferredTier = resolveChatTier(req.user, permissions);
  if (!inferredTier) throwError('Chat access denied.', 403);
  ...
  try {
    ...
    const providerResponse = await streamChatCompletion({
      ...
      maxTokens: chatRequest.policy.maxTokens,
      messages: [{ role: 'system', content: chatRequest.systemPrompt }, ...chatRequest.messages],
    ...
    if (!providerResponse.ok) {
      ...
      res
        ...
        .send(body || JSON.stringify({ message: 'Chat provider request failed.' }));
      return;
    }
