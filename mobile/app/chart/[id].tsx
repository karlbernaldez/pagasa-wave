      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.topBar}>
          ...
          <View style={styles.headingCopy}>
            <Text numberOfLines={1} style={[styles.heading, { color: theme.colors.text }]}>{chart.shortTitle}</Text>
            <Text numberOfLines={1} style={[styles.headingMeta, { color: theme.colors.textMuted }]}>{chart.publishedAt}</Text>
          </View>
        ...
        <GlassSurface style={styles.segmentedControl} strong>
          {displayModes.map((mode, index) => (
            <View key={mode} style={[styles.segment, index === 0 && { backgroundColor: theme.colors.accent }]}> 
              <Text style={[styles.segmentText, { color: index === 0 ? '#FFFFFF' : theme.colors.textMuted }]}>{mode}</Text>
            </View>
          ))}
        </GlassSurface>
...
const styles = StyleSheet.create({
  ...
  landMass: { position: 'absolute', width: 180, height: 270, borderRadius: 90, backgroundColor: 'rgba(210,245,230,0.45)', top: 80, left: 80, transform: [{ rotate: '-14deg' }] },
  landMassTwo: { width: 100, height: 160, top: 245, left: 25, opacity: 0.7 },
  windLineOne: { position: 'absolute', width: 300, height: 160, borderRadius: 150, borderWidth: 2, borderColor: 'rgba(255,255,255,0.38)', top: 40, left: -80 },
  windLineTwo: { position: 'absolute', width: 320, height: 180, borderRadius: 160, borderWidth: 2, borderColor: 'rgba(255,255,255,0.28)', bottom: 20, right: -120 },
  legend: { position: 'absolute', right: 14, top: 18, backgroundColor: 'rgba(2,12,31,0.48)', borderRadius: 16, padding: 12 },
  ...
  resetButton: { position: 'absolute', left: 14, top: 14, flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 12, paddingVertical: 9, borderRadius: 999 },
  ...
  metrics: { flexDirection: 'row', borderTopWidth: StyleSheet.hairlineWidth, borderBottomWidth: StyleSheet.hairlineWidth, marginVertical: 18, paddingVertical: 16 },
  metric: { flex: 1 },
  ...
  primaryButton: { minHeight: 52, borderRadius: 18, flexDirection: 'row', gap: 9, alignItems: 'center', justifyContent: 'center', marginTop: 16 },
