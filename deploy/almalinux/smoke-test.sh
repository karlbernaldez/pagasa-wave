...
SMOKE_BODY="$(mktemp /tmp/wavelab-smoke-body.XXXXXX)"
...
if [[ "${EUID}" -ne 0 ]]; then
  echo "Run this production smoke test with sudo/root so it can read $BACKEND_ENV."
  exit 1
fi

if [[ ! -r "$BACKEND_ENV" ]]; then
  echo "Backend environment file is missing or unreadable: $BACKEND_ENV"
  exit 1
fi
...
check_http() {
  ...
  local expected="$3"
  local code
  code="$(curl --silent --show-error --output "$SMOKE_BODY" --write-out '%{http_code}' --max-time 15 -H "Host: $PUBLIC_HOST" "$url")"
  if [[ ! "$code" =~ $expected ]]; then
    echo "Smoke test failed: $name returned HTTP $code"
    sed -n '1,20p' "$SMOKE_BODY" || true
    return 1
  fi
