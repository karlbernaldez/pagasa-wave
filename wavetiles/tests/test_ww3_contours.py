...
from ww3_contours import generate_contours  # noqa: E402
from ww3_style import color_for_height  # noqa: E402


def test_generate_contours_labels_and_colors_match_raster_palette():
    ...
    assert document["properties"]["variable"] == "hs"
...
def test_integer_contours_are_marked_major():
    lat = np.array([0.0, 1.0], dtype=np.float64)
    lon = np.array([0.0, 1.0], dtype=np.float64)
    data = np.array([[0.0, 2.0], [0.0, 2.0]], dtype=np.float32)

    document = generate_contours(lat, lon, data, levels=np.array([1.0, 1.5]))
    by_height = {feature["properties"]["height"]: feature for feature in document["features"]}

    assert by_height[1.0]["properties"]["major"] is True
    assert by_height[1.5]["properties"]["major"] is False
